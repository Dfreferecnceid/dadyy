...
# FIXED: Combined decorator with credit check for charge commands
def auth_and_free_restricted(func):
    ...
    async def wrapper(client, message):
        ...
        clean_command = command_text.lstrip('/').lstrip('.').lstrip('$').lower()

        # ========== GROUP CHECKS ==========
        if message.chat.type in [ChatType.GROUP, ChatType.SUPERGROUP]:
            # Check if it's EXACTLY /add or /rmv (group management commands)
            # Not /addpx or /rmvpx
            if clean_command == "add" or clean_command == "rmv":
...
━━━━━━━━━━━━━""")
            ...
            # Free users CANNOT use any gate/charge/auth commands in private
