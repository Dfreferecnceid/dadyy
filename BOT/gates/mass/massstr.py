...
# Import all stripe gate checkers from the stripe directory
from BOT.gates.charge.stripe.scharge import StripeChargeChecker as Stripe10Checker, logger, load_users, is_user_banned, check_cooldown, get_user_plan
from BOT.gates.charge.stripe.scharge1 import StripeCharge1Checker as Stripe1Checker
from BOT.gates.charge.stripe.scharge3 import StripeCharge3Checker as Stripe3Checker
from BOT.gates.charge.stripe.scharge012 import StripeCharge012Checker as Stripe012Checker
...
async def parse_card_list_from_command(message):
    ...
    for i, line in enumerate(lines):
        ...
        if i == 0 and any(line.startswith(prefix) for prefix in ['/mstr', '.mstr', '$mstr']):
...
class MassStripeAutoChecker:
    def __init__(self):
        # Initialize all gate checkers
        self.gates = [
            {
                'name': 'Stripe 10$',
                'checker_class': Stripe10Checker,
                'gateway_display': 'Stripe Charge 10$ ♻️',
                'id_display': 'Stripe 10$',
                'needs_user_id': False
            },
            {
                'name': 'Stripe 1$',
                'checker_class': Stripe1Checker,
                'gateway_display': 'Stripe Charge 1$ ♻️',
                'id_display': 'Stripe Charge 1$',
                'needs_user_id': False
            },
            {
                'name': 'Stripe 3.50$',
                'checker_class': Stripe3Checker,
                'gateway_display': 'Stripe Charge 3.50$ ♻️',
                'id_display': 'Stripe charge 3.50$',
                'needs_user_id': True
            },
            {
                'name': 'Stripe 0.12€',
                'checker_class': Stripe012Checker,
                'gateway_display': 'Stripe Charge 0.12 ♻️',
                'id_display': 'Stripe Charge 0.12',
                'needs_user_id': True
            }
        ]
        self.retry_count = 0
    ...
    def get_processing_message_dynamic(self, total_cards, checked, hits, declines, errors, username, user_plan):
        ...
        return f"""<b>「$cmd → /mstr」| <b>WAYNE</b> </b>
━━━━━━━━━━━━━━━
🔄 <b>Mass Stripe Auto</b>
...
💎 <b>Plan:</b> {user_plan}"""
    ...
    async def format_card_result(self, result_text, gate_info, card_details, username, user_data):
        ...
        Extracts relevant parts from the full result
        """
        try:
...
"""
    ...
    async def send_final_summary(self, client, message, successful, failed, total_cards, username, elapsed_time, user_data, processing_msg):
        ...
        summary = f"""<b>「$cmd → /mstr」| <b>WAYNE</b> </b>
━━━━━━━━━━━━━━━
<b>[•] Gateway -</b> Mass Stripe Auto
...
<b>[ﾒ] Time:</b> <code>{elapsed_time:.2f} 𝐬</code> |<b>P/x:</b> <code>Mass ⚡️</code></b>"""
    ...
    async def format_mass_response_collective(self, results, gate_infos, card_details_list, successful, failed, total_cards, username, elapsed_time, user_data):
        ...
        response = f"""<b>「$cmd → /mstr」| <b>WAYNE</b> </b>
━━━━━━━━━━━━━━━
<b>[•] Gateway -</b> Mass Stripe Auto
<b>[•] Status -</b> <code>Complete ✅</code>
━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━ ━
...
@Client.on_message(filters.command(["mstr", ".mstr", "$mstr"]))
@auth_and_free_restricted
async def handle_mass_stripe_auto(client: Client, message: Message):
    ...
    try:
        ...
        command_name = command_text.lstrip('/.$') if command_text else "mstr"
...
• Reply to a text file or message containing cards with /mstr
...
━━━━━━━━━━━━━""")
        ...
        can_use, wait_time = check_cooldown(user_id, "mstr")
        if not can_use:
...
━━━━━━━━━━━━━""")
        ...
        mass_checker = MassStripeAutoChecker()
        ...
        for i, card_details in enumerate(card_list):
            ...
            try:
                ...
                # Each checker has its own check_card method signature
                ...
                if "Charged 💎" in result or "Approved ❎" in result or "3D SECURE❎" in result or "3D Secure ❎" in result:
                    successful += 1
                ...
                # Create error result
                error_result = f"""<b>「$cmd → /mstr」| <b>WAYNE</b> </b>
━━━━━━━━━━━━━━━
<b>[•] Card-</b> <code>{card_details}</code>
